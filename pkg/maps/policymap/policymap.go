...
// prefixMaskMap is the map of all the possible prefix length masks
// of the port field in the policy key, corresponding to the port
// length of that mask. The "16" prefix len implies a full mask,
// and the default value should be "0" anyways.
var prefixMaskMap = map[uint32]uint16{
	0:  0xffff,
	1:  0x7fff,
	2:  0x3fff,
	3:  0x1fff,
...
func (key *PolicyKey) PortProtoString() string {
	...
	case prefixLen > NexthdrBits && prefixLen < FullPrefixBits:
		...
		portLen := prefixMaskMap[portPrefixLen]
