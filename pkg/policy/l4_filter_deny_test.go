...
func TestL3DenyRuleShadowedByL3DenyAll(t *testing.T) {
	...
	state := traceState{}
	resDeny, err := shadowRule.resolveIngressPolicy(td.testPolicyContext, &ctxToA, &state, NewL4PolicyMap(nil), nil, nil)
	require.NoError(t, err)
	...
	state = traceState{}
	resDeny, err = shadowRule.resolveIngressPolicy(td.testPolicyContext, toFoo, &state, NewL4PolicyMap(nil), nil, nil)
	require.NoError(t, err)
...
func TestMergingWithDifferentEndpointSelectedDenyAllL7(t *testing.T) {
	...
	state := traceState{}
	resDeny, err := selectDifferentEndpointsDenyAllL7.resolveIngressPolicy(td.testPolicyContext, &ctxToA, &state, NewL4PolicyMap(nil), nil, nil)
	require.NoError(t, err)
	require.NotNil(t, resDeny)
	require.EqualValues(t, expected, resDeny)
	require.Equal(t, 1, state.selectedRules)
	...
	state = traceState{}
	resDeny, err = selectDifferentEndpointsDenyAllL7.resolveIngressPolicy(td.testPolicyContext, toFoo, &state, NewL4PolicyMap(nil), nil, nil)
	require.NoError(t, err)
	require.Nil(t, resDeny)
	require.Equal(t, 0, state.selectedRules)
	require.Equal(t, 0, state.matchedRules)
...
func TestL3AllowRuleShadowedByL3DenyAll(t *testing.T) {
	...
	expectedDeny := NewL4PolicyMap(map[string]*L4Filter{"80/TCP": {
		Port:     80,
		Protocol: api.ProtoTCP,
		U8Proto:  6,
		wildcard: td.wildcardCachedSelector,
